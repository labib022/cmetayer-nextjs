...
export default function LaundryBookSuccess() {
  const router = useRouter();
  const [serviceDate, setServiceDate] = useState("");

  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("laundrySuccess") || "{}");
    setServiceDate(saved.serviceDate || "");
    localStorage.removeItem("laundrySuccess");
  }, []);
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#F0F0F0] px-4 py-10">
      <div className="w-full max-w-120 flex flex-col items-center gap-8 text-center"
        ...
        <div className="flex flex-col gap-3">
          <h1 className="font-rethink font-medium leading-[130%] tracking-[-1.248px] m-0" style={{ color: "#0F172B", fontSize: "32px" }}>Booking Confirmed!</h1>
