...
import { useSubmitCleaningBookingMutation } from "../../../../../lib/redux/features/cms/cmsApi";
...
export default function CleaningBookStep2() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitCleaningBooking({
        ...
        email:              data.email,
        phone:              data.phone,
        bedrooms:           savedData.bedrooms           || 1,
        bathrooms:          savedData.bathrooms          || 1,
        services_category:  savedData.serviceCategory    || "standard_clean",
        frequency:          savedData.frequency          || "one_time",
        cleaning_date:      savedData.serviceDate        || "",
      }).unwrap();

      // ✅ submit হলে serviceDate save করো success page এর জন্য
      localStorage.setItem("cleaningSuccess", JSON.stringify({ serviceDate: savedData.serviceDate }));
      localStorage.removeItem("cleaningData");
      router.push("/services/cleaning/book/success");
    } catch (err) {
      ...
      <div className="w-full max-w-135 flex flex-col gap-9 relative"
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-4">
            <h1 className="font-rethink font-bold leading-[130%] tracking-[-1.248px] m-0" style={{ color: "#0F172B", fontSize: "32px" }}>Book a Cleaning</h1>
        ...
        <div className="flex items-center gap-4 w-full">
          <button onClick={() => router.push("/services/cleaning/book/step-1")}
